...
export default function Recoveries() {
  ...
  return <div className="page-enter mx-auto max-w-[1440px]">
        ...
        <div className="w-full max-w-md rounded-2xl border border-card-border bg-card p-6 shadow-2xl">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div>
              ...
              <select value={newRootCause} onChange={(e) => setNewRootCause(e.target.value)} className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm">
    ...
    <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-border bg-card p-3 shadow-[0_10px_24px_hsl(221_34%_15%_/_0.03)] lg:flex-row"><SearchBox value={query} onChange={setQuery} /><SelectControl value={status} onChange={setStatus} options={['All statuses', 'Success', 'In progress', 'Pending']} /><SelectControl value={strategy} onChange={setStrategy} options={['All strategies', 'Retry', 'SMS+Retry', 'Email', 'Discount']} /><div className="lg:ml-auto"><SelectControl value={sort} onChange={setSort} options={['Newest first', 'Oldest first', 'Highest amount']} /></div><Button variant="secondary" onClick={exportData} disabled={!rows.length}><Download size={15} /> Export</Button></div>
    ...
    {result.isLoading ? <Skeleton className="h-72" /> : result.isError ? <EmptyState title="Recoveries unavailable" description="The API server could not return recovery records." action={<Button variant="secondary" onClick={() => result.refetch()}><RotateCcw size={14} /> Try again</Button>} /> : rows.length === 0 ? <EmptyState title="No recoveries found" description="New failed payments will appear here after the Razorpay webhook is configured." action={<Button variant="secondary" onClick={() => { setQuery(''); setStatus('All statuses'); setStrategy('All strategies'); }}>Clear filters</Button>} /> : <div className="overflow-hidden rounded-2xl border border-card-border bg-card shadow-[0_10px_24px_hsl(221_34%_15%_/_0.035)]"><div className="hidden grid-cols-[1.5fr_1fr_.75fr_1fr_130px_40px] gap-4 border-b border-border bg-muted/45 px-5 py-3 eyebrow text-muted-foreground md:grid"><span>Customer</span><span>Strategy</span><span>Amount</span><span>Detected</span><span>Status</span><span /></div>{rows.map((item) => <Link href={`/recoveries/${item.id}`} key={item.id} className="grid grid-cols-[1fr_auto] items-center gap-3 border-b border-border/70 px-4 py-4 transition-colors last:border-0 hover:bg-muted/45 md:grid-cols-[1.5fr_1fr_.75fr_1fr_130px_40px] md:gap-4 md:px-5"><div className="flex min-w-0 items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-full bg-secondary text-[11px] font-bold text-secondary-foreground">{initials(customerLabel(item))}</div><div className="min-w-0"><p className="truncate text-xs font-bold">{customerLabel(item)}</p><p className="truncate font-mono text-[10px] text-muted-foreground">{item.payment_id}</p></div></div><span className="hidden text-xs text-muted-foreground md:block">{uiStrategy(item.strategies)}</span><span className="font-mono text-xs font-bold">{formatMoney(item.amount)}</span><span className="hidden text-xs text-muted-foreground md:block">{formatDetected(item.created_at)}</span><StatusBadge status={uiStatus(item.status)} /><MoreHorizontal size={17} className="hidden text-muted-foreground md:block" /></Link>)}</div>}
