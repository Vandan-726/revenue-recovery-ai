...
import { useGetDashboard, useListRecoveries } from '@workspace/api-client-react';
...
export default function Dashboard() {
  ...
  return <div className="page-enter mx-auto max-w-[1440px]">
    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow mb-3 text-primary">Recovery operations</p><h1 className="text-[30px] font-bold tracking-[-.04em] sm:text-[38px]">Good morning<span className="text-primary">.</span></h1><p className="mt-2 max-w-lg text-sm text-muted-foreground">Your recovery engine is connected to live payment data.</p></div><Button variant="secondary" onClick={handleRefresh}><RotateCcw size={14} /> Refresh view</Button></div>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_.9fr]">
      <section className="rounded-2xl border border-card-border bg-card p-5 shadow-[0_10px_24px_hsl(221_34%_15%_/_0.035)] sm:p-6">
        ...
        <div className="flex items-end justify-between gap-4"><div><p className="metric-number text-4xl font-bold">{formatCompact(data.revenue_recovered)}</p><p className="mt-2 text-xs text-primary"><TrendingUp size={13} className="mr-1 inline" /> Live API data</p></div><TinySparkline points={trend.length ? trend : [0]} /></div>
    ...
    <div className="mt-8"><SectionHeading eyebrow="Live queue" title="Active recoveries" action={<Link href="/recoveries" className="inline-flex items-center gap-1.5 text-xs font-bold text-primary hover:underline">See all <ArrowRight size={14} /></Link>} />
      {active.isLoading ? <Skeleton className="h-48" /> : activeRows.length === 0 ? <EmptyState title="No active recoveries" description="New failed payments will appear here once the webhook is configured." /> : <div className="overflow-hidden rounded-2xl border border-card-border bg-card shadow-[0_10px_24px_hsl(221_34%_15%_/_0.035)]">{activeRows.map((recovery) => <Link href={`/recoveries/${recovery.id}`} key={recovery.id} className="grid grid-cols-[1fr_auto] items-center gap-3 border-b border-border/70 px-5 py-4 transition-colors last:border-0 hover:bg-muted/50 sm:grid-cols-[1.4fr_1fr_.8fr_1fr_110px] sm:gap-4"><div className="flex min-w-0 items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-full bg-secondary text-[11px] font-bold text-secondary-foreground">{initials(customerLabel(recovery))}</div><div className="min-w-0"><p className="truncate text-xs font-bold">{customerLabel(recovery)}</p><p className="truncate text-[11px] text-muted-foreground">{recovery.payment_id}</p></div></div><span className="hidden text-xs text-muted-foreground sm:block">{uiStrategy(recovery.strategies)}</span><span className="font-mono text-xs font-bold">{formatMoney(recovery.amount)}</span><span className="hidden text-xs text-muted-foreground sm:block">{formatDetected(recovery.updated_at)}</span><StatusBadge status={uiStatus(recovery.status)} /></Link>)}</div>}
    </div>
    <div className="mt-8 grid gap-4 pb-8 sm:grid-cols-3"><div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4"><Clock3 className="text-chart-3" size={18} /><div><p className="font-mono text-sm font-bold">API-backed</p><p className="text-[11px] text-muted-foreground">metrics refresh on demand</p></div></div><div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4"><TrendingUp className="text-primary" size={18} /><div><p className="font-mono text-sm font-bold">{data.recovery_rate.toFixed(1)}%</p><p className="text-[11px] text-muted-foreground">current recovery rate</p></div></div><div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4"><Coins className="text-chart-2" size={18} /><div><p className="font-mono text-sm font-bold">{formatCompact(data.revenue_at_risk)}</p><p className="text-[11px] text-muted-foreground">total at-risk volume</p></div></div></div>
  </div>;
}