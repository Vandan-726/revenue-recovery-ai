...
import { Activity, BarChart3, ChevronLeft, ChevronRight, CreditCard, LayoutDashboard, Menu, Pause, Play, Radio, Settings, ShieldCheck, X } from 'lucide-react';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { isStreaming, countdown, pauseStream } = useLiveStream();
    ...
    <div className="noise min-h-[100dvh] bg-background">
      <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col bg-background text-foreground border-r border-border md:border-r-0 md:bg-sidebar md:text-sidebar-foreground transition-all duration-300 ${collapsed ? 'w-[248px] md:w-[76px] -translate-x-full md:translate-x-0' : 'w-[248px] -translate-x-full md:translate-x-0'} ${mobileOpen ? '!translate-x-0 shadow-2xl' : ''}`}>
        <div className="flex h-[72px] items-center border-b border-border md:border-sidebar-border px-5">
          ...
          <button onClick={closeMobile} className="ml-auto rounded-lg p-2 text-foreground/60 hover:bg-muted hover:text-foreground md:text-sidebar-foreground/60 md:hover:bg-sidebar-accent md:hover:text-sidebar-foreground md:hidden" aria-label="Close navigation" data-testid="button-close-nav"><X size={18} /></button>
        </div>
        <div className={`px-4 pt-7 ${collapsed ? 'md:px-3' : ''}`}>
          <p className={`eyebrow mb-3 px-2 text-foreground/40 md:text-sidebar-foreground/40 transition-opacity ${collapsed ? 'md:opacity-0' : ''}`}>Workspace</p>
          <nav className="space-y-1">
            {navItems.map(({ href, label, icon: Icon }) => {
              const active = location === href || (href === '/recoveries' && location.startsWith('/recoveries/'));
              return <Link key={href} href={href} onClick={closeMobile} className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-[13px] font-semibold transition-all ${active ? 'bg-sidebar-primary text-sidebar-primary-foreground shadow-[0_8px_18px_hsl(var(--sidebar-primary)_/_0.25)]' : 'text-foreground/75 hover:bg-muted hover:text-foreground md:text-sidebar-foreground/65 md:hover:bg-sidebar-accent md:hover:text-sidebar-foreground'} ${collapsed ? 'md:justify-center md:px-0' : ''}`} data-testid={`link-nav-${label.toLowerCase()}`}><Icon size={18} strokeWidth={active ? 2.4 : 1.8} /><span className={`whitespace-nowrap transition-all ${collapsed ? 'md:hidden' : ''}`}>{label}</span>{label === 'Recoveries' && !collapsed && <span className={`ml-auto rounded-md px-1.5 py-0.5 font-mono text-[10px] ${active ? 'bg-sidebar-primary-foreground/15' : 'bg-muted md:bg-sidebar-accent'}`}>{totalFailed ? (totalFailed >= 1000 ? `${(totalFailed / 1000).toFixed(1)}k` : totalFailed) : '0'}</span>}</Link>;
            })}
          </nav>
        </div>
        <div className="mt-auto p-4">
          <div className={`mb-4 rounded-2xl border border-border md:border-sidebar-border bg-card/80 md:bg-sidebar-accent/70 p-4 ${collapsed ? 'md:hidden' : ''}`}>
            <div className="mb-2 flex items-center justify-between"><span className="eyebrow text-foreground/50 md:text-sidebar-foreground/45">Recovery health</span><span className="size-2 rounded-full bg-sidebar-primary shadow-[0_0_0_4px_hsl(var(--sidebar-primary)_/_0.25)]" /></div>
            <p className="font-mono text-lg font-bold text-foreground md:text-sidebar-foreground">{recoveryRate ? `${recoveryRate.toFixed(1)}%` : '100%'}</p>
            <p className="mt-1 text-[11px] leading-4 text-foreground/50 md:text-sidebar-foreground/50">Live calculated health</p>
          </div>
          ...
          <Link href="/settings" onClick={closeMobile} className={`mt-4 flex items-center gap-3 border-t border-border md:border-sidebar-border pt-4 transition-all hover:opacity-85 ${collapsed ? 'md:justify-center' : ''}`} title="Profile & Account Settings" data-testid="sidebar-profile-link">
            ...
            <div className={`min-w-0 transition-all ${collapsed ? 'md:hidden' : ''}`}><p className="truncate text-xs font-semibold text-foreground md:text-sidebar-foreground">{userName}</p><p className="truncate text-[11px] text-foreground/50 md:text-sidebar-foreground/45">{userEmail}</p></div>
          </Link>
        </div>
      </aside>
      ...
      <main className={`min-h-[100dvh] w-full max-w-full overflow-x-hidden min-w-0 transition-[padding-left] duration-300 pl-0 ${collapsed ? 'md:pl-[76px]' : 'md:pl-[248px]'}`}>
          ...
          <div className="ml-auto flex items-center gap-2 sm:gap-3 shrink-0">
            {isStreaming && (
              <div className="flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-bold text-primary animate-in fade-in duration-200">
                ...
                <button
                  onClick={pauseStream}
                  className="ml-1 rounded p-0.5 text-muted-foreground transition-colors hover:bg-primary/20 hover:text-foreground"
        ...
        <div className="shell-grid min-h-[calc(100dvh-72px)] w-full max-w-full overflow-x-hidden p-5 md:p-9">{children}</div>
      </main>
    </div>
  );
}